	...
	if running:
		...
		if i == threshold: # save cycles by only running this every 10 frames
			...
			if past == class_idx: # rudimentary smoothing by requiring two consecutive detections to be the same
				...
				if class_desc != pastDisp and not thread.is_alive(): # if the definition has changed and the there isn't a pending query
					thread = threading.Thread(target=update_definition, args=(class_desc,))
					thread.start()
				pastDisp = class_desc
				threshold = 10
			else:
		...
		bio = io.BytesIO()
		img.save(bio, format="PNG") # slow but seems to be the only way to make gui behave
